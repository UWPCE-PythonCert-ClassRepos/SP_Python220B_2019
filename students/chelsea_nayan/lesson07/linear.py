'''A module that inputs csv file data into MongoDB linearly'''
...
def import_products(directory_name, product_file):
    ...
    return (attempts, initial_num, db.products.count_documents({}), p_overall_time, product_error)
...
def import_customers(directory_name, customer_file):
    ...
    return (attempts, initial_num, db.customers.count_documents({}), c_overall_time, customer_error)
...
def import_rentals(directory_name, rental_file):
    ...
    # Attempt to import rental data file into MongoDB db, create rental collection
    try:
        with open(customer_file_path, encoding='utf-8-sig') as file:
            reader = csv.DictReader(file)
            for row in reader:
    ...
    return (attempts, initial_num, db.rentals.count_documents({}), r_overall_time, rental_error)
#
