...
def build_test_csvs():
    with open('customers.csv', 'w') as csv_file:
        ...
        file_writer.writerow(['user001', 'Guy Dudeman', '1139 Bro Street', '800-123-4567', 'Guy_Dudeman01@gmail.com'])
    ...
    with open('rentals.csv', 'w') as csv_file:
        file_writer = csv.writer(csv_file, delimiter=',')
        file_writer.writerow(['rental_id', 'product_id', 'customer_id', 'amount', 'time', 'price', 'total'])
...
class TestBasicOps(TestCase):
    ...
    def test_drop_collections(self):
        """test building a table, check if it exists, then drop it and make sure it reuturns nothing"""
        database = database_setup()
        test_dicts = [{'person': 'Maveric'},
                      {'person': 'Charlie'},
                      {'person': 'Iceman'},
    ...
    def test_show_rentals(self):
        ...
        for rental in rentals:
            ...
            csv_results = [next(cust for cust in customers if cust["user_id"] == rental['customer_id'])]
            self.assertEqual(query_results, {customer.pop('user_id'): customer for customer in csv_results})
        database.test.drop()
        delete_test_csv()
    ...
    def test_show_available_products(self):
        ...
        self.assertEqual(show_available_products(), {product.pop('product_id'): product for product in csv_results})
