...
import csv
import os
from linear import import_csv, import_data, drop_collections, MongoDBConnection
from linear import show_rentals, show_available_products, insert_into_table
...
def build_test_csvs():
    ...
    with open('customers.csv', 'w') as csv_file:
        ...
        file_writer.writerow(['user001', 'Guy Dudeman',
                              '1139 Bro Street', '800-123-4567', 'Guy_Dudeman01@gmail.com'])
    ...
    with open('rentals.csv', 'w') as csv_file:
        file_writer = csv.writer(csv_file, delimiter=',')
        file_writer.writerow(['rental_id', 'product_id',
                              'customer_id', 'amount', 'time', 'price', 'total'])
...
class TestBasicOps(TestCase):
    ...
    def test_insert_into_table(self):
        ...
        for test_value in test_dicts:
            query = database.test.find(
                filter={"person": test_value['person']}, projection={'_id': False})
    ...
    def test_show_rentals(self):
        ...
        for rental in rentals:
            ...
            csv_results = [next(cust for cust in customers if cust["user_id"]
                                == rental['customer_id'])]
