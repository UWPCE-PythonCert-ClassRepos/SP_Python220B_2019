...
def main_menu(user_prompt=None):
    '''Main input loop, continues until a valid selection is input'''
...
def get_price(item_code):
    '''Fetches the real time price for an item from a seperate module'''
    print("Get price")
    return get_latest_price(item_code)
    #market_prices.get_latest_price(item_code)

def add_new_item():
    '''Prompts the user to fill out a new item to the inventory'''
...
def item_info():
    '''Allows the user to look up an item by its item code'''
    item_code = input("Enter item code: ")
