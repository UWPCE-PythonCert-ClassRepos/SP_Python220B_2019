#!/usr/bin/env python
'''
Lesson09 assignment (Lesson02 assignment) using decorators to turn on/off logging

pylint disabled (global-statement)

Strategy - add in a new parser.add_argument - choices=['ON', 'OFF'], default on ON
Decorate the functions to make it do enable/disable logging
...
def parse_cmd_arguments():
    ...
    parser.add_argument('-toggle', '-t', type=str, default='ON', required=False,
                        choices=['ON', 'OFF'], help='Toggles logging on or off')
    try:
...
def toggle_logging(func):
    '''A decorator to turn logging ON/OFF for a particular function'''
    def wrapper(*args, **kwargs):
        ...
        if ARGS.toggle == 'OFF':
            ...
            func(*args, **kwargs)
            # Putting logging.disable(logging.NOTSET) <-- Does not work
...
if __name__ == "__main__":
    ...
    logging.disable(logging.NOTSET) # Seems a little clunky
