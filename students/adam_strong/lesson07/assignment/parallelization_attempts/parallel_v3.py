#!/usr/bin/env python
""""
Lesson 07 - Import csv into database (linear)
...
def insert_in_mongo():
    ...
    with mongo:
        ...
        product_thread = threading.Thread(target=add_to_database, args=(products_col, PRODUCTS, 'products'))
        product_thread.start()

        logger.info('Starting importing Customers to MongoDB')
        customer_thread = threading.Thread(target=add_to_database, args=(customers_col, CUSTOMERS, 'customers'))
        customer_thread.start()      

        logger.info('Starting mporting Rentals to MongoDB')
        rentals_thread = threading.Thread(target=add_to_database, args=(rentals_col, RENTALS, 'rentals'))
    ...
    custprod_tuple = [RETURN_TUPLES['customers'], RETURN_TUPLES['products']] # Puts customer/product tuple in a list
