...
def import_data(directory, product_csv, customer_csv, rentals_csv):
    ...
    # Old return statement:: return [tuple(IMPORTED), tuple(FAILED)]
    return output_tuple
...
def insert_in_mongo():
    ...
    with mongo:
        ...
        logger.info('Importing Products to MongoDB:')
        prodCount = loop.run_until_complete(add_to_database(products_col, PRODUCTS)) ### Adding asyncio.run
        logger.info('Importing Customers to MongoDB:')
        custCount = loop.run_until_complete(add_to_database(customers_col, CUSTOMERS))
        logger.info('Importing Rentals to MongoDB:')
        loop.run_until_complete(add_to_database(rentals_col, RENTALS))
    ...
    custprod_tuple = [custCount, prodCount] # Puts customer/product tuple in a list
    return custprod_tuple


async def add_to_database(collection, chosen_data): #### ADDING async
    ...
    #i = await collection.count_documents({}) # How many records in this collection?
    count = 0
    error = 0
    for row in chosen_data:
        try:
            count += 1
            ins = await collection.insert_one(dict(row)) ##### ADDING await
            logger.debug(ins)
        except TypeError:
            error += 1
    ...
    logger.info('Tuple : %s', str(col_tuple))
    return col_tuple
