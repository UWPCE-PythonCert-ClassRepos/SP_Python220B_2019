...
def timer(func):
    ...
    def run_time(*args, **kwargs):
        '''to time the function run_time'''
        start_time = datetime.datetime.now().timestamp()
        run_funct = func(*args, **kwargs)
        time = (datetime.datetime.now().timestamp() - start_time)
        with open('timings.txt', 'a+') as file:
            file.write('{} processed {:10} database with {:8} files in {:11.7f} seconds.'.
                       format(func.__name__, args[1], run_funct[0], time) + '\n')
        return run_funct
    return run_time
