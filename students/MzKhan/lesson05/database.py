...
def import_data(directory_name, product_file, customer_file, rentals_file):
    ...
    with mongo:
        ...
        # Delete the existant collections if exist.
        products.drop()
        customers.drop()
        rentals.drop()
        try:
...
def show_available_products():
    ...
    with mongo:
        database = mongo.connection["HPNorton"]
        available_products = database.Product.find(
...
def show_rentals(product_id):
    ...
    with mongo:
        database = mongo.connection.HPNorton
        renters = database.Rental.find({'product_id':product_id})
