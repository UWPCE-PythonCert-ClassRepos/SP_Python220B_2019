...
class TestCaseFour(unittest.TestCase):
    """Test the import of large files for timing of
    function execution data to be written to log file"""
    def test_large_file_import(self):
        """Tests importing of data from csv files"""
        current_dir = os.getcwd()
        path = current_dir + r"\data_files"
        clear_db()
        import_test = import_data(path, "customer_data_large.csv",
                                  "product_data_large.csv", "rental_data_large.csv")
        self.assertEqual(list(import_test), [(1000, 1000, 1000), (0, 0, 0)])
