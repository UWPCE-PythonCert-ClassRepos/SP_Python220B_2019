# Lesson 10 assignment HP Norton "Meta programming" Activity, created by Niels Skvarch
"""
...
def function_timer(func):
    """Decorator function for the timing of function executions"""
    def timer(*args, **kwargs):
        start_time = time.time()
        func_data = func(*args, **kwargs)
        total_time = time.time() - start_time
        LOGGER.warning("the function %s took %s to execute", func.__name__, total_time)
        return func_data
    return timer
...
def import_data(directory, customer_data_file, product_data_file, rental_data_file):
    ...
    try:
        with open(customer_file_path) as file:
            ...
            LOGGER.warning("adding file %s to the database", customer_file_path)
            for i in converted:
    ...
    try:
        with open(product_file_path) as file:
            LOGGER.warning("adding file %s to the database", product_file_path)
            converted = csv.DictReader(file)
            for i in converted:
    ...
    try:
        with open(rental_file_path) as file:
            LOGGER.warning("adding file %s to the database", rental_file_path)
            converted = csv.DictReader(file)
            for i in converted:
    ...
    LOGGER.warning("%s product entries %s customer entries and %s rental"
                   " entries were processed", products.count_documents({}),
                   customers.count_documents({}), rentals.count_documents({}))
