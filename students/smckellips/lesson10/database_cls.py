...
class MongoDBConnection():
    ...
    def __enter__(self):
        ...
        self.db = self.connection[HPNorton.DB_NAME]
        return self
...
class HPNorton():
    '''Code copied intact from lesson05 and wrapped into a class.'''
    DB_NAME = 'Inventory'
    ...
    def import_data(directory_name, product_file, customer_file, rentals_file):
        ...
        for file_name in inv_map:
            with open(f'{directory_name}/{file_name}', 'r') as csv_file:
                ...
                for row in reader:
                    result = HPNorton.insert_db(
                        mongo, inv_map[file_name]['Name'], row)
                    if result:
                        inv_map[file_name]['count'] += 1
                        HPNorton.LOGGER.info("Adding %s to collection %s. ID: %s",
                                             row[inv_map[file_name]['key']],
                                             inv_map[file_name]['Name'],
                                             result)
                    else:
                        inv_map[file_name]['errors'] += 1
                        HPNorton.LOGGER.warning("Failed to add %s to database.",
                                                row[inv_map[file_name]['key']])
