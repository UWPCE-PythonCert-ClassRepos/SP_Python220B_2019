...
def clear_test_collections():
    ...
    with database.MONGO:
        db = database.MONGO.connection.media  # pylint: disable=C0103
...
class DatabaseUnitTest(TestCase):
    ...
    def test_import_data(self):
        ...
        with patch.object(MongoDBConnection, 'set_collection_names', return_value=TEST_DB_NAMES):
            incorrect_names = database.import_data('', 'Products .csv', 'Customers .csv',
                                                   'Rentals .csv')
        ...
        with patch.object(MongoDBConnection, 'set_collection_names', return_value=TEST_DB_NAMES):
            duplicate_data = database.import_data('', 'Products.csv', 'Customers.csv',
                                                  'Rentals.csv')
    ...
    def test_show_rentals(self):
        ...
        with patch.object(MongoDBConnection, 'set_collection_names', return_value=TEST_DB_NAMES):
            ...
            rental_customers_2 = database.show_rentals('prd00002')
            rental_customers_3 = database.show_rentals('prd00003')
        ...
        self.assertEqual({}, database.show_rentals('prod00004'))
...
class DatabaseIntegrationTest(TestCase):
    """
    Class for integration tests for database.py
    """
