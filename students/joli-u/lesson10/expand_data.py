...
def write_data(filename, data, n_entries):
    """
    Writes data to csv file
    """
    header = {'customers': ['user_id', 'name', 'address', 'phone', 'email'],
              'products': ['prod_id', 'description', 'prod_type', 'qty_avail'],
              'rentals': ['prod_id', 'qty', 'user_id']}

    heading = header[filename[6:-4]]
    filename_n = filename[:-4] + str(n_entries) + filename[-4:]

    with open(filename_n, 'w', newline='') as file:
...
def main():
    ...
    n_entries = int(input("Enter number of records to generate: "))
    ...
    # write data to file
    write_data('files/customers.csv', customer_info, n_entries)
    write_data('files/products.csv', product_info, n_entries)
    write_data('files/rentals.csv', rental_info, n_entries)
