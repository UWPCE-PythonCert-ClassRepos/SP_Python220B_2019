...
def add_new_item():
    ...
    if is_furniture.lower() == "y":
        ...
        new_item = furniture_class.Furniture(item_code, item_description, item_price,
                                             item_rental_price, material=item_material,
                                             size=item_size)
    else:
...
def item_info():
    """Prints all the item info. Alters the user if the item is not found."""
    item_code = input("Enter item code: ")
    if item_code in FULL_INVENTORY:
        print_dict = FULL_INVENTORY[item_code]
        for key_word, value in print_dict.items():
            print("{}:{}".format(key_word, value))
    else:
