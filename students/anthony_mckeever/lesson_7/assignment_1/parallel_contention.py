...
# Having a global lock object can force deadlock contention by limiting the
# amount of work that can happen while it's locked.  See more on line 90
LOCK = threading.Lock()

# Ignore the "Too many local variables" message as 5 variables is reasonable
...
def ingest_file(database, directory, file_name, collection, result):
    ...
    # Once this lock is engaged, other threads will not be able to write data
    # to the hp_norton database's various tables until the lock is released.
    # Thus, this negates the benefit supplied by multithreading.  In this case
    # it would be better to move the LOCK.acquire to just before the
    # db_collection.insert_one call and move the LOCK.release call to just
    # just after the db_collection.insert_one call to prevent resource lockout.
    LOCK.acquire()
