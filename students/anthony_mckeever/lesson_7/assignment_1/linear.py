...
#from time import time as time_delta
...
def ingest_file(database, directory, file_name, collection):
    ...
    #   !(int)Number of records in DB prior to run
    #   !(int)Number of records after running
    #   (float)Time to run
...
def import_data(directory_name, product_file, customer_file, rentals_file):
    ...
    products = customers = rentals = None

    with mongo:
        database = mongo.connection.hp_norton

        LOGGER.info("Importing products...")
        products = ingest_file(database,
                               directory_name,
                               product_file,
                               "products")

        LOGGER.info("Importing customers...")
        customers = ingest_file(database,
                                directory_name,
                                customer_file,
                                "customers")

        LOGGER.info("Importing rentals...")
        rentals = ingest_file(database,
                              directory_name,
                              rentals_file,
                              "rentals")
