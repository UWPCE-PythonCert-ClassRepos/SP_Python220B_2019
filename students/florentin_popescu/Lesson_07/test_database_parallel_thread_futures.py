...
Modified on Mon Aug 26 10:22:33 2019
...
import database_parallel_thread_futures as db
from database_parallel_thread_futures import csv_to_mongo_thread
from database_parallel_thread_futures import csv_to_mongo_futures
from database_parallel_thread_futures import main_parallel_thread
from database_parallel_thread_futures import main_parallel_futures
from database_parallel_thread_futures import print_products, drop_data
# ==================================
...
class TestImports(unittest.TestCase):
    ...
    def test_csv_to_mongo_thread(self):
        """
        ...
        for name in dbs:
            ...
            cnt_rec = csv_to_mongo_thread(path, name)
        ...
        for name in dbs:
            ...
            try:
                csv_to_mongo_thread(path_prod, name)
            except (FileNotFoundError, UnboundLocalError) as err:
                LOGGER("file searched_customer_file not found")
                LOGGER(err)
    # -----------------------------

    def test_csv_to_mongo_futures(self):
        """
            testing csv import to MongoDb
        """
        dbs = ["customer", "product", "rental"]
        ...
        for name in dbs:
            path = "csvdata/{}s.csv".format(name)
            cnt_rec = csv_to_mongo_futures(path, name)
            cnt_rec_tpl = tuple(cnt_rec[idx] for idx in range(3))
            expected_cnt_rec_err = (1000, 0, 1000)
            try:
                self.assertEqual(cnt_rec_tpl, expected_cnt_rec_err)
        ...
        for name in dbs:
            path_prod = f"csvdata/searched_{name}_file.csv"
            try:
                csv_to_mongo_futures(path_prod, name)
            except (FileNotFoundError, UnboundLocalError) as err:
                LOGGER("file searched_customer_file not found")
                LOGGER(err)
    # -----------------------------

    def test_main_parallel_thread(self):
        """
            test main parallel thread executor
        """
        LOGGER.info("test loading executor <Thread>")
        path = "csvdata/customers.csv", "csvdata/products.csv"
        colection = "customer", "product"
        thread = main_parallel_thread(path, colection)
        res_tpl = (tuple(thread[0][idx] for idx in range(3)),
                   tuple(thread[1][idx] for idx in range(3)))
        exp_res_tpl = ((1000, 0, 1000), (1000, 0, 1000))
        self.assertEqual(res_tpl, exp_res_tpl)
        # -----------------------------

    def test_main_parallel_futures(self):
        """
            test main futures executor
        """
        LOGGER.info("test loading executor <Futures>")
        path = "csvdata/customers.csv", "csvdata/products.csv"
        colection = "customer", "product"
        future = main_parallel_futures(path, colection)
        res_tpl = (tuple(future[0][idx] for idx in range(3)),
                   tuple(future[1][idx] for idx in range(3)))
        exp_res_tpl = ((1000, 0, 1000), (1000, 0, 1000))
        self.assertEqual(res_tpl, exp_res_tpl)
        # ----------------------------
    ...
    def test_print_products(self):
        ...
        try:
            ...
            load = csv_to_mongo_thread(path, "product")
