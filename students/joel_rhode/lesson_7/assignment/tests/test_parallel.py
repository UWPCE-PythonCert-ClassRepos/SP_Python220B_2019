...
import parallel
from parallel import (import_all_data, read_csv, write_many_to_database, MongoDBConnection)
from tests.test_lists import mock_customer_list, mock_rental_list, mock_product_list


TEST_DIRECTORY = 'sample_csv_files'
TEST_DATABASE = 'parallel_test'
...
class DatabaseUnitTests(TestCase):
    ...
    @patch('parallel.DATABASE')
    def test_import_all_data(self, mock_database):
        """Unit test of the import_all_data function."""
        parallel.DATABASE = TEST_DATABASE
        with patch('parallel.read_csv', side_effect=[(mock_product_list, 2),
                                                     (mock_customer_list, 1),
                                                     (mock_rental_list, 0)]) as mock_read_csv:
            with patch('parallel.write_many_to_database', side_effect=[(7, 2), (5, 1), (8, 15)]) \
                    as mock_write_many:
                result = import_all_data(TEST_DIRECTORY, 'test_products.csv',
                                         'test_customers.csv', 'test_rentals.csv')
                mock_write_many.assert_called_with(parallel.DATABASE, 'rentals', mock_rental_list)
