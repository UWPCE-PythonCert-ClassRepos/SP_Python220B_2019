...
# pylint: disable=unused-wildcard-import,wildcard-import,invalid-name,too-few-public-methods,wrong-import-order,singleton-comparison,too-many-arguments,logging-format-interpolation,too-many-locals,no-else-return,unused-variable,redefined-builtin,broad-except,no-else-raise
...
def retry(num_attempts=3, exception_class=Exception, log=None, sleeptime=1):
    '''Built retry logic into program thru a decorator.  Core pymongo seems to have one built-in
       but wanted to use this as something to improve the program'''
    def decorator(func):
        @functools.wraps(func)
        def wrapper(*args, **kwargs):
            for i in range(num_attempts):
                logging.info(f'Here is the number of attempt {num_attempts}')
                try:
                    ...
                    if i == num_attempts - 1:
                        ...
                        if log:
                            logging.info('Failed with error %r, trying again', e)
...
class MongoDBConnection():
    ...
    @retry(5, ConnectionError, logging)
    def __init__(self, host='127.0.0.51', port=27017):
