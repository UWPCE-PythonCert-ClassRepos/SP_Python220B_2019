...
class PopulateTables(TestCase):
    ...
    def test_display_customer_table(self):
        """Return all customers remaining in table"""
        LOGGER.info('Show remaining customers in customer table')
        all_customers = Customer.select()
        listed_customers = [customer.name + " " + customer.last_name for customer in all_customers]

        self.assertEqual(display_customer_table(), listed_customers)
