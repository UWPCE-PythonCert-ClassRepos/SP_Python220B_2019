...
# pylint: disable=too-many-locals,pointless-statement,logging-format-interpolation
import os
import csv
import logging
import time
...
def time_function(func):
    """ New function to time other functions """
    def setup_logger(*args, **kwargs):
        ...
        logging.info('Func: {}'.format(func.__name__))
        logging.info('Time: {}'.format(func_time))
        if func.__name__ == 'import_data':
            num_records = sum(output[0])
            logging.info('Records processed: {}'.format(num_records))
        return output
    return setup_logger
...
def show_rentals(product_id):
    ...
    with mongo:
        ...
        for row in rentals.find({'product_id': product_id}):
            ...
            customer_dict[result['user_id']] = {'name': result['name'],
...
if __name__ == '__main__':
    import_data('input_csv', 'products_s.csv', 'customer_s.csv', 'rentals_s.csv')
    import_data('input_csv', 'products_m.csv', 'customer_m.csv', 'rentals_m.csv')
    import_data('input_csv', 'products_l.csv', 'customer_l.csv', 'rentals_l.csv')
    show_available_products()
    show_rentals('product_l')
