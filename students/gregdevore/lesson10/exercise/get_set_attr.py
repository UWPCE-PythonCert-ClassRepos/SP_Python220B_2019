...
def update_person(person):
    while True:
        att = input("What would you like to update for:\n"
                    "{}\n"
                    '(type "quit" to quit, or "del" to delete an attribute) >> '.format(person)
                    )
        if att.strip().lower() == "quit":
            break
        elif att.strip().lower() == 'del':
            att = input("Which attribute would you like to delete? >> ")
            if hasattr(person, att):
                print('Deleting attribute {}'.format(att))
                delattr(person, att)
            else:
                print("This person does not have an attribute {}".format(att))
        else:
