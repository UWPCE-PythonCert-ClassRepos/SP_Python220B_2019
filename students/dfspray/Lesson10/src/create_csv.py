...
def write_csv(quantity):
    ...
    product_data = expand_product_data(quantity)

    customer_data = expand_customer_data(quantity)

    rentals_data = expand_rentals_data(quantity)
...
def expand_product_data(quantity):
    """This method expands the product_data"""
    product_data = {'prd00001': {'description': 'TV', 'product_type': 'livingroom',
                                 'quantity_available': 3},
                    'prd00002': {'description': 'Couch', 'product_type': 'livingroom',
                                 'quantity_available': 1},
                    'prd00003': {'description': 'Chair', 'product_type': 'livingroom',
                                 'quantity_available': 0}}
    for number in range(4, quantity):
        key = 'prd{}'.format('0'*(5-len(str(number)))+str(number))
        product_data.setdefault(key, {'description': "something{}".format(str(number)),
                                      'product_type': "type{}".format(str(number)),
                                      'quantity_available': number})
    return product_data

def expand_customer_data(quantity):
    """This method expands the customer_data"""
    customer_data = {'cst00001': {'name': 'Charlie', 'address': '123 Fleet Street',
                                  'phone_number': '1231234123'},
                     'cst00002': {'name': 'Andrey', 'address': '123 Jordan Street',
                                  'phone_number': '4564564567'},
                     'cst00003': {'name': 'Vijay', 'address': '123 Lake Street',
                                  'phone_number': '7987897891'}}

    for number in range(4, quantity):
        key = 'cst{}'.format('0'*(5-len(str(number)))+str(number))
        customer_data.setdefault(key, {'name': "something{}".format(str(number)),
                                       'address': "somewhere{}".format(str(number)),
                                       'phone_number': str(number)})
    return customer_data

def expand_rentals_data(quantity):
    """This method expands the rentals_data"""
    rentals_data = {'cst00001': {'name': 'Charlie', 'rentals': ['prd001', 'prd002']},
                    'cst00002': {'name': 'Andrey', 'rentals': ['prd002', 'prd003']},
                    'cst00003': {'name': 'Vijay', 'rentals': ['prd001', 'prd003']}}
