...
import logging
import unittest
from src import parallel_mp

LOG_FORMAT = "%(asctime)s %(filename)s:%(lineno)-3d %(levelname)s %(message)s"
LOG_FILE = 'test_parallel_mp.log'
...
class TestDatabase(unittest.TestCase):
    ...
    def test_import(self):
        ...
        parallel_mp.delete_database()

        #Fully successful import
        actual_tuples1 = parallel_mp.import_data('csvs', 'product_data.csv', 'customer_data.csv',
                                                 'rentals_data.csv')
        ...
        actual_tuples2 = parallel_mp.import_data('csvs', 'produc_data.csv', 'customer_data.csv',
                                                 'rentals_data.csv')
        ...
        actual_tuples3 = parallel_mp.import_data('csvs', 'product_data.csv', 'custome_data.csv',
                                                 'rentals_data.csv')
        expected_tuples3 = ((1000, 0, 1000, actual_tuples3[0][3]), (0, 0, 0, 0))
        self.assertEqual(actual_tuples3, expected_tuples3)
        parallel_mp.delete_database()

        main_return = parallel_mp.main()
