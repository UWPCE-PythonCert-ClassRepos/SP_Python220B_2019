...
def _validate_credit_limit(limit):
    try:
        ...
        logger.error("Invalid parameter: 'credit_limit' is not numeric.")
        logger.error(f"{e}")
        return 0.
...
def all_records() -> Customer:
    """generator for returning record(s) from the database"""
...
def list_active_customers() -> tuple:
    """This function will return tuple of Customer objects
    whose status is currently active."""
    with database.transaction():
        # NOTE:  MUST USE "==" (operator style testings).  Pythonic "is" fails
        return tuple([c for c in all_records() if c.active is True])


def list_active_customer_ids() -> tuple:
    """This function will return a tuple of customer IDs for
    customers whose status is currently active."""
    return tuple([c.customer_id for c in list_active_customers()])
