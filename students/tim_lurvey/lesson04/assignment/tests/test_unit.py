...
class BaseTestCase(TestCase):
    ...
    def populate_db_with_records(self):
        ...
        [basic_operations.add_customer(*record) for record in RECORDS]
    ...
    def test_list_active_customers(self):
        ...
        my_active_records = [basic_operations.search_customer(rec[CUSTOMER_ID]) \
                             for rec in RECORDS if rec[ACTIVE] is True]

        list_active = [basic_operations.search_customer(c.customer_id) \
                       for c in basic_operations.list_active_customers()]

        self.assertEqual(first=my_active_records,
                         second=list_active)

        self.assertEqual(first=tuple(r.get('customer_id') for r in my_active_records),
                         second=basic_operations.list_active_customer_ids())
