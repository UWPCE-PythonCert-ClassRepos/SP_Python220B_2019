...
def calculate_additional_fields(data):
    """
    Calculate and set new values needed for each entry in the data set.  Each calculation is isolate
    into their own function for clarity.
    ...
    - Any value calculation errors that can be overwritten as a 0, will be 0 upon errors.  A WARNING
      will be written to a log file if run in debug mode.
    - Values that are critical and cannot be missing will report errors and the entry will be skipped.
    ...
    for key, value in data.items():
        ...
        for sub_key in ['product_code', 'rental_start', 'rental_end', 'price_per_day', 'units_rented', ]:
            try:
                read = value[sub_key]
