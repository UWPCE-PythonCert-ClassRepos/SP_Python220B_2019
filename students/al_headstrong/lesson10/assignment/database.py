...
def function_performance(func):
    def inner(*args, **kwargs):
        ...
        LOGGER.info('Function: %s\tTime taken: %s\tRecords processed: %s',
...
if __name__ == '__main__':
    ...
    clear_database()
    import_data('sample_csv_files/', 'products_long.csv', 'customers_long.csv', 'rentals_long.csv')
    show_available_products()
    show_rentals('prd50')
    import_data('sample_csv_files/', 'products_short.csv', 'customers_short.csv', 'rentals_short.csv')
    show_available_products()
    show_rentals('prd50')
    clear_database()