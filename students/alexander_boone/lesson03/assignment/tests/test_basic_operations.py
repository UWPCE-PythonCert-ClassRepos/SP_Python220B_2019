...
class TestBasicOperations(TestCase):
    ...
    def test_add_customer(self):
        ...
        with database.transaction():
            alex = Customer.get(Customer.customer_id==12345)
            self.assertEqual(alex.customer_id, 12345)
            self.assertEqual(alex.name, 'Jeff')
            self.assertEqual(alex.lastname, 'Bezos')
            self.assertEqual(alex.home_address, '3582 Main St')
            self.assertEqual(alex.phone_number, 1234567890)
            self.assertEqual(alex.email_address, 'Jeff@amazon.com')
            self.assertEqual(alex.status, 'active')
            self.assertEqual(alex.credit_limit, 200000)
    
    def test_failure_add_customer(self):
        '''Test adding a customer without enough inputs.'''
        with self.assertRaises(TypeError):
            add_customer(12345, 'Alex', 'Boone')
