'''
    Assignment 9, Context Manager development
    For this section, I added logging and exception handling to the context manager.
...
class MongoDBConnection():
    ...
    def __init__(self, host='127.0.0.1', port=27017):
        ...
        LOGGER.info('__init__(host=%s, port=%s)', self.host, self.port)

    def __enter__(self):
        try:
            LOGGER.info('__enter__ MongoDBConnection')
            self.connection = MongoClient(self.host, self.port)
            LOGGER.info('MongoDBConnection successful')
        except Exception as error:
            LOGGER.error('Error connecting to Mongo DB, %s', error)
            raise
        return self

    def __exit__(self, exc_type, exc_val, exc_tb):
        self.connection.close()
        LOGGER.info('__exit__ MongoDBConnection')
