...
def add_customer(customer_id, firstname, lastname, home_address, \
    ...
    try:
        ...
        new_customer.save()
        LOGGER.info(f"Successfully added {firstname} {lastname} to customers.db")
    except peewee.IntegrityError:
        LOGGER.error(f'Unique customer_id required. Customer {customer_id} not added')
...
def search_customer(customer_id):
    ...
    try:
        ...
        customer_dict[customer_id] = [cust_data.first_name, cust_data.last_name, \
            cust_data.email_address, cust_data.phone_number]
...
def list_active_customers():
    ...
    active_customers = int(Customer.select().where(Customer.customer_status).count())
