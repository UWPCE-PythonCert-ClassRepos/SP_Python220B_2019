...
def import_products(directory_name, filename):
    """Import product list from *.csv file into a MongoDB database."""
    ...
    # Removing the call to provide the output data to the queue, even though
    # the queue is still expecting the call.
    #output_queue.put((num_attempts, initial_num, db.products.count_documents({}),
    #                  prod_end-prod_start), error_prod)


def import_customers(directory_name, filename):
    """Import customers from a *.csv file into a MongoDB database."""
    ...
    output_queue.put((num_attempts, initial_num, db.customers.count_documents({}),
                      cust_end-cust_start), error_cust)


def import_rentals(directory_name, filename):
    """Import rental information from a *.csv file into a MongoDB database."""
    ...
    output_queue.put((num_attempts, initial_num, db.rentals.count_documents({}),
...
if __name__ == '__main__':
    ...
    # Open the queue for collecting the thread output data.
    output_queue = Queue()
    # List for storting the data from the queue as it is popped from the output queue.
    queue_storage = []
    # List of threads to be added and run.
    threads = []
    ...
    for thread in threads:
        thread.start()
        # Popping the output data from the threads into the list.
        queue_storage.append(output_queue.get())

    parallel_end = time.time()

    print('Total Time =', parallel_end-parallel_start)
    print(queue_storage)
