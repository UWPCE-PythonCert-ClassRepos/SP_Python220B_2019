...
def main_menu(user_prompt=None):
    """
    Presents main menu
    :param user_prompt: Value entered on command line.
    """
...
def add_new_item():
    ...
    if is_furniture.lower() == "y":
        ...
        new_item = furniture_class.Furniture(item_code,\
                    item_description, item_price, item_rental_price, item_material, item_size)
    else:
...
def item_info():
    """Asks the user for an item code and prints the item information."""
    item_code = input("Enter item code: ")
    if item_code in FULL_INVENTORY:
        print_dict = FULL_INVENTORY[item_code]
        for k, _v in print_dict.items():
            print("{}:{}".format(k, _v))
    else:
