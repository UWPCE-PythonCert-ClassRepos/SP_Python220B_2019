...
def timer(function, *args, **kwargs):
    """
    Creates a funtion that wraps another function adn times it
    """
    def add_timer(*args, **kwargs):
        """
        Wraps a timer around a function
        """
        start = time.time()
        return_value = function(*args, **kwargs)
        total_time = time.time() - start
        # Assume that if the function return is a list,
        # then it's contains info about records loaded
        f = open("timings.txt", "a")
        if isinstance(return_value, list):
            record_count = 0
            for value in return_value[0]:
                record_count += value
            f.write(f"{function.__name__} took {total_time:.3f} seconds and loaded {record_count} records.\n")
        else:
            f.write(f"{function.__name__} took {total_time:.3f} seconds.\n")
        f.close()
        return return_value
    # Return the function the timer
    return add_timer


class Timer(type):
    """
    A metaclass that wraps the functions of its classes
    with a timer to time how long the function takes.
    """
...
class Database(metaclass=Timer):
