...
def my_logger_decorator(func):
    """Decorator to set the logging level """
    def my_logger(level, *args, **kwargs):
...
if __name__ == "__main__":
    ...
    RAW_DATA = load_rentals_file(ARGS.log_level, ARGS.input)
    logging.debug('Got data from %s file', (ARGS.log_level, ARGS.input))
    NEW_DATA = calculate_additional_fields(ARGS.log_level, RAW_DATA)
    save_to_json(ARGS.log_level, ARGS.output, NEW_DATA)
    logging.debug('New data added to %s file', (ARGS.log_level, ARGS.output))
