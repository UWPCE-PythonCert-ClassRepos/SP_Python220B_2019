...
def import_data(directory_name, customer_file, product_file, rentals_file):
    ...
    with mongo:
        ...
        thread_generate01 = threading.Thread(target=csv_handler.generate_document_list,
                                             args=[product_file, "product"])
        thread_generate02 = threading.Thread(target=csv_handler.generate_document_list,
                                             args=[customer_file, "customer"])
        thread_generate03 = threading.Thread(target=csv_handler.generate_document_list,
                                             args=[rentals_file, "rentals"])
        thread_insert01  = threading.Thread(target=products.insert_many)
        thread_insert02  = threading.Thread(target=products.insert_many)
        thread_insert03  = threading.Thread(target=products.insert_many)

        profiler.enable()
        try:
            thread_generate01.start()
            thread_generate01.join()
