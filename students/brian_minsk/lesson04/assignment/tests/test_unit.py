...
class PostPopulatedTest(TestCase):
    ...
    def test_update_multiple_customers_credit(self):
        """ Try to update the credit of multiple customers but one of the customers is not in the
        database.

        Note: This shows iterating over a generator for lesson04 (update_multiple_customers_credit
        uses a generator expression to return an generator).
        """
        # Use a new credit limit of 20,000 for all.
        ids_credit = tuple((id, 20000) for id in range(1, 5))
        ...
        # Check the ones that got updated that they were updated to 20000.00.
        for customer_id in range(1, 4):
            acustomer = Customer.get(Customer.customer_id == customer_id)
            self.assertEqual(float(acustomer.credit_limit), 20000.00)
