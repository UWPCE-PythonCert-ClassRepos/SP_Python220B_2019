...
def my_timer(func):
    """ Closure meant to be use as a decorator that times how long func takes to run.
    Writes the function name, elapsed_time, and number of records processed to a file called
    "timings.txt"
    """
        ...
        try:
            with open("timings.txt", 'a') as timing_file:
                line = "{}, {}, {}\n".format(func.__name__, elapsed_time,
                                             return_value["num_docs_total"])
                timing_file.write(line)
        except KeyError:
            print("my_timer only works with works with functions that return a dict with a \
                'num_docs_total' key.")
        return return_value
    return timer_wrap
...
def import_data(directory_name="csv_files", product_file="product.csv",
    ...
    return {"num_docs_by_type": (n_products, n_customers, n_rentals),
            "num_errors": (errs_products, errs_customers, errs_rentals),
            "num_docs_total": n_products + n_customers + n_rentals}
...
def show_available_products():
    ...
    return {"available_products": available_products, "num_docs_total": num_docs}
...
def show_rentals(product_id):
    ...
    return {"customer_info": customer_info, "num_docs_total": num_docs}
...
if __name__ == "__main__":
    ...
    import_data("csv_files", "product.csv", "customers.csv", "rentals.csv")
    show_available_products()
    show_rentals(4)
    import_data("csv_files", "product100.csv", "customers100.csv", "rentals100.csv")
    show_available_products()
    show_rentals(4)
    import_data("csv_files", "product10000.csv", "customers10000.csv", "rentals10000.csv")
    show_available_products()
    show_rentals(4)
