...
#pylint: disable=W1202, R0201, R0914, C0123, W0613, C0103, R1704

logging.basicConfig(level=logging.ERROR)
LOGGER = logging.getLogger(__name__)


TIMER_LOG_FILE = "timings.txt"


def timefunc(fn, *args, **kwargs):
    """this is to measure the function running time"""
    def fncomposite(*args, **kwargs):
        start_time = datetime.now()
        rt = fn(*args, **kwargs)
        time_elapsed = datetime.now() - start_time
        exec_string = f'Executing {fn.__name__}, took {time_elapsed} seconds.'
        print(exec_string)
        if fn.__name__ == "import_data":
            p_count, c_count, r_count = rt[0]
            my_string = f'Records processed: Products: {p_count}, Customers: {c_count}, \
Rentals: {r_count}'
            print(my_string)
        elif fn.__name__ == "access_csv":
            my_string = f'Reading from file: {args[1]}, number of records: {len(rt)}'
            print(my_string)
        with open(TIMER_LOG_FILE, "a+", newline="") as file:
            file.write(f'{exec_string}\n{my_string}\n')
        return rt
...
class Database(metaclass=Timed):
    """Class to take in metaclass timed to measure elapsed time"""

    def access_csv(self, file_name):
    ...
    def import_data(self, directory_name, product_file, customer_file, rentals_file):
        ...
        with mongo:
            ...
            try:
                customer_list = self.access_csv(customer_csv)
            ...
            try:
                prod_list = self.access_csv(products_csv)
            ...
            try:
                rental_list = self.access_csv(rentals_csv)
            ...
            (prod_errors, cust_errors, rentals_errors)
...
if __name__ == "__main__":
    ...
    TIMED = Database()
    MY_COUNTS, MY_ERRORS = TIMED.import_data(DIRECTORY_NAME, PRODUCT_FILE,
