...
def add_to_coll(coll_name, data_list):
    """
...
def import_data(directory_name, product_file, customer_file, rentals_file):
    ...
    with mongo:
        ...
        prod_list = csv_to_dict(product_path)
        cust_list = csv_to_dict(customer_path)
        rent_list = csv_to_dict(rental_path)


        prod_thread = threading.Thread(target=add_to_coll,
                                       args=(products, prod_list))
        prod_thread.start()
        products_count = len(prod_list)
        logging.info("Successfully added product data to database.")


        cust_thread = threading.Thread(target=add_to_coll,
                                       args=(customers, cust_list))
        cust_thread.start()
        customers_count = len(cust_list)
        logging.info("Successfully added customer data to database.")

        rent_thread = threading.Thread(target=add_to_coll,
                                       args=(rentals, rent_list))
        rent_thread.start()
