...
# logging.basicConfig(filename="db.log", filemode="w", level=logging.INFO)
# LOGGER = logging.getLogger(__name__)
...
def add_customer(**kwargs):
    ...
    try:
        ...
        LOGGER.error(f"Customer id {kwargs['customer_id']} is already in use.")
        raise
...
def update_customer_credit(customer_id, customer_credit_limit):
    ...
    with database.transaction():
        try:
            ...
            LOGGER.info(f'Customer {customer_id} credit limit updated')
            cust.save()
