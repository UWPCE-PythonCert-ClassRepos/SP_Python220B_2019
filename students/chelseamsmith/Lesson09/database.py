...
class MongoDBConnection():
    ...
    def __enter__(self):
        ...
        self.database = self.connection.store
            ...
            with open(os.path.join(directory_name, rentals_file)) as csvfile:
                ...
                for row in rentals_reader:
                    ...
                    rental_info = {"product_id": row[0], "customer_id": row[1],
                                   ...
                                   "cost_per_day": row[4]}
                    mongo.rentals.insert_one(rental_info)
        except:
            rentals_errors += 1
...
def show_available_products():
    ...
    with mongo:
        ...
        result = mongo.database.product.find({"quantity_available": {"$gte": "1"}})
        for row in result:
...
def show_rentals(product_id):
    ...
    with mongo:
        ...
        result = mongo.database.rentals.find({"product_id": product_id})
        for row in result:
            query = mongo.database.customer.find_one({"customer_id": row["customer_id"]})
