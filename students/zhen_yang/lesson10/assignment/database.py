...
# using decorator metaprogramming to add timing information to the
# HP Norton application.
def timing_decorator(func):
    """  create a timing decorator to add timing information to
         all the functions of HP Norton application. It gather
         timing data and write it to a file called timings.txt
         This file should contain function name, time taken
         and number of records processed.
    """
    def timing_decorator_warpper(*args, **kwargs):
        # add the timing stuff.
        start = time.time()
        res = func(*args, **kwargs)
        end = time.time()
        # write the time information to the 'timing.txt' file.
        # includes: function name, time taken, number of records processed.
        with open('timings.txt', 'a') as outfile:
            if func.__name__.startswith('import'):
                outfile.write('\n-------- Start Timing Information --------\n')
                outfile.write(f'Input Files: {args[1]}, {args[2]}, {args[3]}\n')
                outfile.write('Number of records processed: '
                              f'{res[0][0]+ res[0][1] + res[0][2]}\n')
            outfile.write(f'Func:{func.__name__}()')
            outfile.write('    {:.2f} second\n'.format(end - start))
            if func.__name__.startswith('show_rentals'):
                outfile.write('------- End Timing Information -------\n\n')
        return res
    return timing_decorator_warpper
