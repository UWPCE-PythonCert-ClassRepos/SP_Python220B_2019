...
cmds = [] # store the command line options

def logging_decorator(func):
    """ This function defines the decorator for logging selection. """
...
def parse_cmd_arguments():
    ...
    logging.debug('-- Adding 4 command line arguments --')
    parser.add_argument('-i', '--input', help='input JSON file', required=True)
    parser.add_argument('-o', '--output', help='ouput JSON file', required=True)
    # add single command line variable that can turn logging on or off for
    # decorated classes or functions.
    parser.add_argument('-l', '--logging', help='turn on logging for'
                        'decorated functions by setting -l option',
                        default=False, action='store_true')
    # add 'debug' option
    parser.add_argument('-d', '--debug', help='increate debug levels',
                        default=0, type=int, choices=[0, 1, 2, 3])
    logging.debug('-- Finish adding 4 command line arguments --')
...
def load_rentals_file(filename):
    ...
    with open(filename) as file:
        try:
            data = json.load(file)
            logging.debug(f'-- open input file:{filename} successfully --')
...
if __name__ == "__main__":
    cmds = parse_cmd_arguments()
    old_data = load_rentals_file(cmds.input)
    new_data = calculate_additional_fields(old_data)
    save_to_json(cmds.output, new_data)
